...
def val(trainer: Trainer,device,opt,epoch,total_iters,logger,dataloader,path,metric):
    ...
    if dict(log_dict_val.items())[opt.metric]>metric:
...
def main():
    ...
    whole_steps=batch_num * opt.epochs if len(train_dataloader)%opt.batch_size == 0 else batch_num+1
