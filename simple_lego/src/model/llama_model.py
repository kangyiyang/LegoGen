...
class Text_Cross_Atten(nn.Module):
    def __init__(
            self,
            layer_id: int,
            params: ModelArgs,
        ...
        self.text_norm=RMSNorm(params.text_dim,params.norm_eps)
    ...
    def forward(
        ...
        x_norm=self.norm(x)
        texts=self.text_norm(texts)

        xq, xk, xv = self.wq(x_norm), self.wk(texts), self.wv(texts)
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs, model: CLIP = None):
        """
        ...
        """
        super().__init__()
        pretrain=False if model is not None else True
        ...
        if model is not None:
            ...
            self.text_layers = torch.nn.ModuleList()
            for layer_id in range(params.n_layers):
                self.text_layers.append(Text_Cross_Atten(
                                            layer_id=layer_id,
                                            params=params
                                            ))

        self.token_embeddings = ParallelEmbedding(params.vocab_size, int(params.dim/2))
        self.choose_embeddings = ParallelEmbedding(params.max_seq_len, int(params.dim/2))
        ...
        self.output_choose = MLP(params.dim, params.max_seq_len)
    ...
    def forward(
        ...
        h_c=self.choose_embeddings(chooses)
        # h:batch_size*pad_seq_len*token_embedding_dim
        # h_p:batch_size*pad_seq_len*choose_embedding_dim
        h=torch.cat((h,h_c),dim=2)
        ...
        if texts is not None and len(texts) > 0:
            ...
            text_embeddings=self.text_model.encode_text(text_tokens)
        ...
        #     # 在generation中使用pad_mask还是有些问题，加上cache len后
        #     mask = torch.where(pad_mask.unsqueeze(1) == 0, float('-inf'), mask)
            
        
        for i in range(len(self.layers)):
            h = self.layers[i](h, start_pos, freqs_cis, mask, autoregressive)
            if texts is not None and len(texts) > 0 and (i+1) % self.add_cross==0:
                h=self.text_layers[i].forward(h,text_embeddings,freqs_cis)
            
            
        h = self.norm(h)
        
        output_id=self.output_id(h).float()
        output_choose=self.output_choose(h).float()

        return output_id, output_choose
